...
| App preferences, selected/default/last station, adult age-screen result, accepted Terms version, community-content visibility, station-scoped blocked identities, Chat-mention station opt-ins, and UI state | No developer or analytics server | On-device preferences only; date of birth is not saved; all app-private data is excluded from Android cloud backup and device-to-device transfer | Out of scope for collection | Core/optional by setting | App functionality; safety | No |
| Local Chat-mention classification and duplicate suppression | No additional transfer; processes the already-loaded selected-station Chat snapshot | Exact signed-in-name comparison occurs on-device; first snapshot is a baseline; only bounded SHA-256 message fingerprints remain temporarily in memory; notification text contains station and sender but no message text | On-device processing is outside collection | Optional and station-scoped | App functionality | No |
| Explicitly generated diagnostic snapshot: app/build, Android/API and coarse device model, station, bounded playback/error category, network state, broad output category, and up to five allowlisted transitions | No automatic transfer and no developer backend; Copy uses Android's clipboard and Share opens the user-controlled system chooser | Preview/history are memory-only; no raw errors, route names, stable identifiers, community/account content, endpoints, or logs | On-device processing is outside collection; verify whether the active form treats the chosen recipient transfer under the user-initiated sharing exception | Optional and explicitly initiated | App functionality; support | Provisionally No under the specific user-initiated-action exception; reconcile against the active Console form |
