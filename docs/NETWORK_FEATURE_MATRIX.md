...
| Sleep Timer | N/A | N/A | N/A | N/A | N/A | Yes | No | No | Local playback/service state | No | Planned M24 | No current implementation; expiry and lifecycle behavior must remain service-owned |
| Cast / audio-output selection | Stream authorization and route support require discovery | Same | Same | Same | Same | Potentially | No for listening | No | Android route stack and possibly an authorized Cast receiver path | No | Planned M25, discovery first | Do not expose a route UI until local/remote handoff, dependency, receiver, and station authorization implications are verified |
