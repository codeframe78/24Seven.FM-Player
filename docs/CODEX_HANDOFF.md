...
- Latest successful validation: GitHub Android CI passes all 117 unit-test methods, lint, and debug assembly. The established 24/24 API 35 emulator suite and earlier 21/21 Android 16 Razr suite remain green; focused Pixel Fold folded-portrait, media-notification navigation, multi-field library search, generated 1,500-track rendering/sorting, and preserved full-list Razr checks also pass.
- Latest local M23 refinement: trusted chat emoticons and the compact More hierarchy remain intact; the notification body reopens the existing player task, folded portrait keeps controls and the station carousel visible, library searches preserve Title/Album/Artist/Genre, Library/Favorites can sort by play state, and indexed availability matching keeps a full 1,500-track Favorites list responsive.
...
- Known blockers: Queue rows lack stable track IDs. M17 Private Messages remains deferred after Inbox/Sent discovery and a profile-originated MorgHubby send appeared successful but was not delivered; the recipient account confirmed absence and the site owner was notified. M23 upload signing, off-PC recovery, public privacy hosting, Play App Signing, objective declarations, public support contact, and Music & Audio classification are complete. The owner selected an 18+ audience consistent with the adult station community and existing chat; Console still requires least-privileged reviewer credentials before that declaration can be saved.
...
- Next concrete task: supply reusable least-privileged Play reviewer credentials through Console, save the selected 18+ target audience, complete content rating/Data Safety, and prepare the internal-test release.
...
- Latest production implementation commit: `bdb533a` (`Improve library search playback and favorites`).
- Latest repository automation checkpoint: `0f61089` (`Restore Discord roadmap progress updates`). No stream URL, credential, or trust-policy expansion was introduced by these changes.
...
M23 is now divided into M23.1–M23.7 release-artifact, UGC, API 36, Play-review, rights, device, and launcher-quality gates. M23.3 is complete: the app targets API 36; all 117 unit tests, lint/release bundle checks, and 27/27 API 36 connected tests pass; a cold launch and two Back actions reached the existing exit dialog. See `docs/m23-api36-readiness.md`. M23.2 UGC research is complete in `docs/m23-ugc-safety-research.md`: current Play policy, all five public station Terms/Contact surfaces, the absence of live-message report IDs and user-block support, and mature-content risk are mapped. Do not implement a report transport until station administration confirms that the Contact form is authorized and monitored for app-originated abuse reports. The next action is the owner's decision to retain UGC with that confirmation and the full native Terms/report/block safeguards, or to exclude Chat/request attribution from the Play candidate.
...
M20 Adagio.FM certification is complete. The unchanged primary relay reached live playback on the physical Razr,
...
test prevents Adagio from inheriting SST-only request-message or listener-activity flags. A representative MorG
session signed in natively, restored after a forced process restart without Keystore errors, remained isolated from
the other stations, loaded authenticated empty Favorites, exposed the Chat composer and one green requestable
