...
| Adagio.FM (`adagio`) | Independent Adagio.FM root/form/CAPTCHA | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout | Fully station-scoped session and host validation | M20 challenge/CAPTCHA form loads natively; representative account sign-in/restore/logout remains interaction-blocked |
| Death.FM (`death`) | Independent Death.FM root/form/CAPTCHA; native read-only challenge verified on wired device | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout | Fully station-scoped session and host validation | Native adapter implemented; `RIP_Subscribe` membership branding/route modeled; representative authenticated evidence pending |
...
- Live sign-in and restored-session behavior still require representative accounts and user-entered security challenges for M20–M22; M19 is complete. No credentials or challenge answers belong in test fixtures.
