...
Updated July 15, 2026. The five account systems are independent even where their legacy pages and field names are similar. No credential, active cookie, CAPTCHA token, private identifier, or authentication header is recorded here.
...
| Adagio.FM (`adagio`) | Independent Adagio.FM root/form/CAPTCHA; representative native sign-in verified | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; process-restart restoration verified without Keystore errors | M20 complete: MorG sign-in, protected restoration, station isolation, authenticated Favorites/Chat/request availability, and station-only logout verified |
| Death.FM (`death`) | Independent Death.FM root/form/CAPTCHA; representative native sign-in verified | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; process-restart restoration verified without Keystore errors | M21 complete: Morgue sign-in, protected restoration, station isolation, authenticated Favorites/Chat/request availability, and station-only logout verified; `RIP_Subscribe` remains a separate browser route |
| Entranced.FM (`entranced`) | Independent Entranced.FM root/form/CAPTCHA; representative native sign-in verified | Same station-relative registration module | Exact recovery route not yet separately verified | Same station-relative account/edit module | Same station-relative logout verified | Separate station ID, expected host, cookie manager entry, encrypted storage key, and auth state; process-restart restoration verified without Keystore errors | M22 complete: MorG sign-in, protected restoration, station isolation, authenticated Favorites/Chat/request availability, and station-only logout verified |
...
- Membership and request cooldown are enabled only for SST. Representative M19–M22 certification did not expose a reliable native membership/cooldown contract for 1980s.FM, Adagio.FM, Death.FM, or Entranced.FM, so those capabilities remain disabled.
