...
- Membership and request cooldown are enabled only for SST. Representative M19–M21 certification did not expose a reliable native membership/cooldown contract for 1980s.FM, Adagio.FM, or Death.FM, so those capabilities remain disabled; Entranced.FM awaits M22 evidence.
- Live sign-in and restored-session behavior still require a representative account and user-entered security challenge for M22; M19–M21 are complete. No credentials or challenge answers belong in test fixtures.
