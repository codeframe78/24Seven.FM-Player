...
- M20 reconfirmed the existing Adagio.FM extended Queue endpoint at HTTP 200 with separate bounded Queue/Played tables, both unchanged station-supplied audio relays, and the existing read-only artwork, Chat, least-played suggestion, login-challenge, and trusted Forums paths without adding an endpoint or mutation.
