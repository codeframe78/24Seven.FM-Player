...
Updated July 16, 2026 after completing the M23.6 network-loss and playback-reconnection checkpoint. Estimates are active Codex elapsed time in this environment, including inspection, implementation, Gradle validation, documentation, Git, and remote confirmation—not traditional human developer time.
...
| M23.6 Release device matrix and pre-launch report | M, accessibility/network/device checkpoints green | 3–6 hours | Medium | Emulator coverage passes across API 26, API 36, 16 KB, Fold, and Tablet states; large-text reflow, one-shot network restoration, and physical Razr hinge checks pass. Complete Play delivery/update and pre-launch remediation | Play crawler/delivery access and spoken TalkBack pass |
...
The M23.6 network-loss and playback-reconnection checkpoint completed July 16, 2026. Offline terminal failures now wait for validated connectivity and perform exactly one automatic Media3 prepare while preserving user pause/stop/station-change intent; unrelated online server failures do not loop. Live phone and Tablet cut/restore cycles returned to `Playing live` without another tap. The 130-test unit suite, 40/40 Tablet connected suite, and debug lint pass. See [m23-network-recovery-validation.md](m23-network-recovery-validation.md).
