...
- Status: in progress. M23.1's non-secret audit is green with explicit backup exclusions, native open-source notices, 126 unit tests, lint, release builds, and 16 KB APK/ELF checks; protected signing and Play delivery remain. M23.3 API 36 readiness and M23.7 launcher polish are complete. M23.2 is authorized and its native age/Terms/reveal, report, block, privacy, and duplicate-safe delivery implementation is green; only administrator receipt reconciliation remains. M23.6 current-head 36/36 suites pass on API 26, API 36, the 16 KB runtime, Pixel Fold open, and Pixel Tablet landscape; focused changed-path checks also pass after Fold half-open/closed and Tablet portrait transitions. M17 Private Messages remains explicitly deferred and is not shipped.

#### M23 sub-milestone plan

| Milestone | Size | Estimate | Usage | Outcome and completion gate | Primary confidence variable |
| --- | --- | --- | --- | --- | --- |
| M23.1 Current-head signed release candidate | M, local audit green | 2–4 hours | Medium | Production manifest, dependencies/notices, backup exclusions, unsigned release builds, and 16 KB APK/ELF packaging pass; regenerate the protected signed AAB from selected HEAD, validate signer/version code, and complete Play-delivered install/update evidence; see `m23-release-candidate-audit.md` | Access to the protected upload identity and whether version code 2 has already reached Play |
...
| M23.6 Release device matrix and pre-launch report | M, current-head local matrix green | 3–6 hours | Medium | Current-head 36/36 suites pass on API 26, API 36, the 16 KB runtime, Pixel Fold open, and Pixel Tablet landscape; focused changed-path checks pass after Fold half-open/closed and Tablet portrait transitions. Complete Razr hinge, Play delivery/update, and pre-launch remediation | Physical Razr posture and Play crawler/delivery access |
