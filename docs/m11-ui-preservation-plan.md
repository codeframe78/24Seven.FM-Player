...
| --- | --- | --- | --- | --- |
| Buildable native baseline | Gradle wrapper, `:app`, CI and Windows validation scripts | API 35 target, API 36 compile platform, Java 17 | Build and workflow validation documented in M1 | Keep toolchain, module, caches, and validators unchanged |
...
| Primary and fallback streams | Bundled PLS resources and Media3 playback preparation | Each station uses its verified primary stream, then its verified fallback | Playback mapping/service tests and M2/M3 device validation | Do not alter URLs or fallback ordering; describe fallback attempts without exposing URLs |
