...
- **Active focus:** M23.2 UGC safety and moderation. The [research checkpoint](docs/m23-ugc-safety-research.md) confirms the Play requirements and a possible station-admin Contact route, but implementation awaits authorization/handling confirmation and the decision to retain or exclude public UGC.
...
| M23.2 UGC safety and moderation | L, split into research and implementation | 6–12 hours after station input | 🚧 Research checkpoint complete ([evidence](docs/m23-ugc-safety-research.md)) | Play requirements and station surfaces are mapped; retain UGC only after an authorized, monitored report destination is confirmed, otherwise remove Chat/request attribution from the Play candidate |
...
M23 is active on this branch. The Google Play developer account and app record are established, Play App Signing is selected, the no-ads/non-government/no-financial/no-health declarations are saved, the public [privacy notice](https://codeframe78.github.io/24Seven.FM-Player/) is live and recorded in Console, and the public support email plus Music & Audio category are saved. The owner selected an 18+ target audience consistent with the adult legacy-station community and existing freeform chat; Console entry remains locked behind reviewer-access setup. Play-ready graphics and device screenshots are prepared; compact/medium/expanded and folded Android layouts, notification-to-app navigation, multi-field library search, play-state sorting, full-list Favorites responsiveness, and API 36 targeting are validated. Protected upload signing and recovery are green, but the recorded signed candidate predates the latest product fixes and must be regenerated from current HEAD. M23.2 research now confirms that public Chat and request attribution need non-skippable Terms plus separate in-app report-content, report-user, and block-user behavior backed by accountable moderation; an 18+ Play selection alone does not replace mature-content safeguards. The report destination and retain/exclude decision remain open. Foreground-service evidence, explicit brand/content rights, broader release-device coverage, reviewer credentials, audience/content declarations, tester setup, Play delivery, and explicit publication authorization also remain gates. M17 Private Messages is not shipped and does not block Alpha testing.
